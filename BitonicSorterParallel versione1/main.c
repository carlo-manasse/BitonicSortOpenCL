...
// b[1] -> log size
	...
	cl_mem mem_buffer_b=clCreateBuffer (context,CL_MEM_READ_ONLY ,sizeof(int)*2,NULL,&err);
	if (err != CL_SUCCESS) {
		printf("Error in create buffer !!!Err code %d\n\n", err );
		exit(0);
	}
	...
	err=clEnqueueWriteBuffer (the_queue,mem_buffer_b,CL_TRUE,0,(size_t) sizeof(int)*2,(void *)b,0,NULL,&event);
	if (err != CL_SUCCESS) {
		printf("Error in clEnqueueWriteBuffer !!!Err code %d\n\n", err );
		exit(0);
	}
	...
	float kerneltime=(end - start) * 1.0e-6f;
